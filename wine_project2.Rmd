...
##All variables and model selection
...
fre = as.data.frame(table(wine.model$country))
...
pro = as.data.frame(table(wine.model$province))
colnames(pro) = c('province',"frequency")
fre_province = pro %>%
  mutate(proportion = frequency/sum(pro$frequency))
fre_province$proportion = percent(fre_province$proportion)
view(fre_province)


var = as.data.frame(table(wine.model$variety))
colnames(var) = c('variety',"frequency")
fre_variety = var %>%
  mutate(proportion = frequency/sum(var$frequency))
fre_variety$proportion = percent(fre_variety$proportion)
view(fre_variety)

win = as.data.frame(table(wine.model$winery))
colnames(win) = c('winery',"frequency")
fre_winery = win %>%
  mutate(proportion = frequency/sum(win$frequency))
fre_winery$proportion = percent(fre_winery$proportion)
view(fre_winery)
...
##model modifying
```{r}
# recoding the level in each variable
wine_recode = wine_ff %>%
  mutate(country_level = recode(country,
                                "US" = "US", "France" = "France", "Italy" = "Italy", .default = "Other")) %>%
  mutate(province_level = recode(province,
                                "California" = "California", "Washington" = "Washington", "Oregon" = "Oregon", "Tuscany" = "Tuscany", .default = "Other")) %>%
  mutate(variety_level = recode(variety,
                                "Pinot Noir" = "Pinot Noir", "Chardonnay" = "Chardonnay", "Cabernet Sauvignon" = "Cabernet Sauvignon", "Red Blend" = "Red Blend", "	
Sauvignon Blanc" = "Sauvignon Blanc", "Bordeaux-style Red Blend" = "Bordeaux-style Red Blend", .default = "Other")) %>%
  select(grade, price, country_level, province_level, variety_level)


wine_recode
```
From the above proportion in each category, we picked the proportion that is over 4% to analyzing. The 'country' variable includes 'US', 'Italy', 'France' and others. The province includes 'California', 'Washington', 'Oregon', 'Tuscany' and others. The 'variety' includes 
'Pinot Noir','Red Blend','Chardonnay','Cabernet Sauvignon','Sauvignon Blanc','Bordeaux-style Red Blend' and others.

```{r}
# modelling using GLM
wine_recode$grade = as.numeric(wine_recode$grade)
model_f = glm(grade ~ ., family = binomial, data=wine_recode)
summary(model_f)
```

Then, we did the GLM model and find out that price, countryItaly, countryUS, ProvinceOther, and ProvinceWashington are significantly important for the wine rating over 90. 


```{r}
broom::glance(model_f)
```
The AIC and BIC are both lower than the previous model with only price or country. Therefore, this model is better. 
...
```{r}
p1 = plot_model(model_f, show.values = TRUE, transform = NULL,
           title = "Log-Odds", show.p = FALSE)
p2 = plot_model(model_f, show.values = TRUE,
